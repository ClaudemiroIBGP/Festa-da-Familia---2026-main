...
// ✅ Endpoint do Google Apps Script (Web App) para gravar na planilha
const SHEETS_ENDPOINT =
  'https://script.google.com/macros/s/AKfycbzZqMHznHXqprPyvPYqVOUbBPr-QiUNjfdJz5hBNa7YZWOFgqhVDeD2T-U7HK9kjodL/exec';

// ✅ Envio mais estável para Apps Script em site estático (evita preflight/CORS do JSON)
async function sendRegistrationToSheets(payload: any) {
  try {
    ...
    // mode:'no-cors' envia mesmo sem CORS liberado.
    // Obs: a resposta vem "opaque" e não dá pra ler status no frontend.
    ...
    console.error('Falha ao enviar inscrição para planilha:', err);
  }
}

const fotos = ['foto1.jpeg', 'foto2.jpeg', 'foto3.jpeg', 'foto4.jpeg'].map(
...
const App = () => {
  ...
  // ✅ NOVAS OPÇÕES
  const [paymentMethod, setPaymentMethod] = useState<'PIX' | 'DINHEIRO' | 'CARTAO_TEMPLO'>('PIX');
  ...
  const handleRegistrationSubmit = (e: React.FormEvent) => {
    ...
    // ✅ Se for pagamento fora do PIX (dinheiro/cartão templo), registra e finaliza sem modal
    if (paymentMethod === 'CARTAO_TEMPLO' || paymentMethod === 'DINHEIRO') {
      const payload = {
        timestamp: new Date().toISOString(),
        paymentMethod,
        paymentStatus: paymentMethod === 'CARTAO_TEMPLO' ? 'pending_cartao_templo' : 'pending_dinheiro',
      ...
      setSuccessMessage({
        ...
        body:
          paymentMethod === 'CARTAO_TEMPLO'
            ...
            : 'Seus dados foram recebidos.\nLembre-se de efetuar o pagamento em dinheiro conforme orientações da organização para garantir sua participação.',
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50">
      {isPaymentModalOpen && (
        <PaymentModal
          ...
          // ✅ Não existe mais PIX programado: sempre false
          isInstallment={false}
          ...
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center space-x-3">
              <div className="grid grid-cols-2 gap-4">
                {fotos.map((src, idx) => (
                  <div key={idx} className="aspect-square rounded-lg overflow-hidden shadow-sm bg-gray-100">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-lg text-gray-700 leading-relaxed mb-6">
                A VI Festa da Família IBGP é um evento especial projetado para fortalecer os laços familiares em um ambiente
                acolhedor e cristão.
              </p>
                ...
                <div className="bg-white rounded-xl p-6 shadow-md mt-6">
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-blue-50 has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
                      ...
                      />
                      <span className="ml-3 text-sm font-medium text-gray-700">PIX</span>
                    </label>
                    ...
                    <label className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-blue-50 has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
                      <input
                        ...
                        className="w-5 h-5 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="ml-3 text-sm font-medium text-gray-700">Dinheiro</span>
                    </label>
                    ...
                    <label className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-blue-50 has-[:checked]:bg-blue-50 has-[:checked]:border-blue-500">
                      <input
                        ...
                        name="paymentMethod"
                        value="CARTAO_TEMPLO"
                        checked={paymentMethod === 'CARTAO_TEMPLO'}
                        onChange={() => setPaymentMethod('CARTAO_TEMPLO')}
                        className="w-5 h-5 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="ml-3 text-sm font-medium text-gray-700">Cartão no Templo</span>
                    </label>
                  </div>
                </div>
